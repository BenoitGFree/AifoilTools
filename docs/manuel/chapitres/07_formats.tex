...
\subsection{Format GNU (\chemin{.gnu})}

Format de points brut compatible avec le logiciel \textbf{Axile} et
directement exploitable par \textbf{gnuplot}. C'est un simple fichier
texte de deux colonnes \texttt{x~y}, sans en-tête ni nom de profil.

\begin{itemize}
    \item Une ligne par point, deux colonnes \texttt{x y}.
    \item Chaque nombre en notation scientifique à 18 décimales
        (\texttt{\%.18e}), séparé par une seule espace.
    \item Fins de ligne \texttt{LF} (y compris après le dernier point),
        garanties même sous Windows.
    \item Aucun en-tête, commentaire ni troisième colonne.
\end{itemize}

\begin{lstlisting}[caption={Exemple de fichier GNU (.gnu)}]
1.000000000000000000e+00 0.000000000000000000e+00
9.754952615109063752e-01 6.041608209341215764e-03
0.000000000000000000e+00 0.000000000000000000e+00
\end{lstlisting}

Ce format est produit à l'identique par \code{numpy.savetxt} et
relisible par \code{numpy.loadtxt} (tableau de forme $(n, 2)$).

\begin{noteinfo}
Le format \chemin{.gnu} est proposé en \textbf{écriture uniquement}
(sauvegarde du profil courant). Pour réimporter des points,
utilisez un fichier \chemin{.dat} (Selig) ou \chemin{.csv}.
\end{noteinfo}
...
\section{Format projet (\chemin{.aftproj})}
\label{sec:aftproj}

Le format \chemin{.aftproj} sauvegarde l'\textbf{intégralité d'une
session de travail} dans un seul fichier, là où les formats
précédents ne sauvegardent qu'un profil isolé. Un projet contient :

\begin{itemize}
    \item le \textbf{profil courant} et le \textbf{profil de
        référence} (avec leur définition spline complète le cas
        échéant) ;
    \item l'\textbf{image de calque} éventuelle, \emph{embarquée}
        dans le fichier, ainsi que sa \textbf{transformation}
        (position, échelle, rotation, visibilité).
\end{itemize}

Il s'agit d'un fichier \textbf{JSON} (texte). L'image est encodée
en base64 et embarquée directement : le projet est donc
\textbf{autonome et portable}. On retrouve l'image du calque à la
réouverture même si le fichier image d'origine a été déplacé ou
supprimé.

\begin{itemize}
    \item Ouverture : \menu{Fichier $\rightarrow$ Ouvrir projet\dots}
        (\touche{Ctrl}+\touche{Maj}+\touche{P}).
    \item Enregistrement : \menu{Fichier $\rightarrow$ Enregistrer
        projet\dots} (\touche{Ctrl}+\touche{Alt}+\touche{S}).
\end{itemize}

\begin{important}
Le format \chemin{.aftproj} est le \textbf{seul} qui conserve
l'image de calque et son calage. Si votre travail comporte un
décalquage (voir section~\ref{sec:calque}), enregistrez un projet
plutôt qu'un simple profil.
\end{important}

\begin{noteinfo}
L'image étant embarquée en base64, un projet comportant une image
volumineuse produit un fichier \chemin{.aftproj} plus gros que la
somme des fichiers de profil. C'est le prix de la portabilité
(aucune dépendance à un fichier image externe).
\end{noteinfo}
...
l'application (NACA~2412 comme profil courant et comme référence).
Pour générer d'autres NACA en cours de session, utilisez le menu
\menu{Fichier $\rightarrow$ Profil NACA $\rightarrow$ Profil
courant\dots} ou \menu{\dots Profil référence\dots} : un dialogue
demande les indices, puis le nombre de points. Voir la procédure
détaillée en section~\ref{sec:naca}, page~\pageref{sec:naca}.

\begin{noteinfo}
Un profil NACA non encore modifié peut être régénéré proprement à
une autre résolution via le menu contextuel \menu{Nombre de
points\dots} (clic droit), qui rappelle la formule analytique au
lieu d'interpoler les points. Voir section~\ref{sec:naca}.
\end{noteinfo}
