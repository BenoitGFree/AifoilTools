...
\begin{noteinfo}
These parameters feed the \textbf{solver selected} in
\menu{Options $\rightarrow$ Solver} (XFoil or FlexFoil, see
chapter~\ref{chap:interface}). Both engines interpret the same settings;
a few options specific to the XFoil executable (\menu{ASEQ} sweep,
re-initialization on failure) have no effect on FlexFoil, which
converges through a native method.
\end{noteinfo}
...
During the simulation, the tab controls are disabled, and a
\textbf{progress bar} appears in the status bar: determinate (one
step per completed airfoil) when several airfoils are simulated,
otherwise indeterminate (a ``busy'' animation), since an
angle-of-attack sweep runs as a single computation with no
intermediate progress. At the end, the \menu{Results} tab is
