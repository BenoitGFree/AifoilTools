...
\begin{enumerate}
    ...
    \item the \textbf{status bar}, at the bottom of the window, where
        information messages and, during simulations, a
        \textbf{progress bar} together with progress messages are
        displayed.
\end{enumerate}
...
\begin{tabularx}{\linewidth}{l l X}
    ...
    Solver / XFoil, FlexFoil
        & ---
        & Selects the aerodynamic \textbf{computation engine} used for
          simulations. The choice is remembered and applies to the next
          run. Submenu shown only if a second solver is available. See
          the box below.\\
    \bottomrule
\end{tabularx}
...
\begin{noteinfo}
\textbf{Aerodynamic solver.} AirfoilTools can rely on two equivalent
computation engines:
\begin{itemize}
    \item \textbf{XFoil} --- Mark Drela's classic executable, bundled
        with the distribution (always available);
    \item \textbf{FlexFoil} --- a native (Rust) reimplementation of the
        same solver, faster to launch because it is called as a library.
        It is offered only if present in the installation.
\end{itemize}
Both produce the same results (polars, $C_p$, boundary layer) up to
numerical differences, and obey the same parameters (\menu{XFoil
Settings} tab). The default setting is \textbf{XFoil}.
\end{noteinfo}
