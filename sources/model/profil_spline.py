...
u"""
Objet metier ProfilSpline aerodynamique 2D.

Variante de Profil qui utilise des BezierSpline (multi-segment) au lieu
de Bezier simples pour definir l'extrados et l'intrados.

Creation::

    # Depuis un fichier
    p = ProfilSpline.from_file('naca2412.dat', fmt='selig', unit='mm')

    # Depuis une designation NACA
    p = ProfilSpline.from_naca('2412', n_points=200)

    # Direct
    p = ProfilSpline(points, name='MonProfil')

    # Approximation spline (1 segment par cote)
...
class ProfilSpline(object):
    u"""Profil aerodynamique 2D avec splines de Bezier.
    ...
    def __init__(self, points, name='Sans nom'):
        ...
        self._spline_extrados = None   # BezierSpline or None
        self._spline_intrados = None   # BezierSpline or None
    ...
    def __repr__(self):
        return "ProfilSpline('%s', %d pts)" % (
            self._name, len(self._points))
    ...
    def points(self):
        ...
        En mode spline, reconstruit le profil complet (convention Selig)
    ...
    def points(self, value):
        ...
        if pts.ndim != 2 or pts.shape[1] != 2:
            raise ValueError(
                ...
                % str(pts.shape))
        self._points = pts
        self._spline_extrados = None
        self._spline_intrados = None
    ...
    def output_format(self):
        u"""Format d'ecriture ('selig', 'lednicer', 'csv', 'bspl')."""
    ...
    def has_splines(self):
        u"""True si le profil est defini par des BezierSpline."""
        return (self._spline_extrados is not None
                and self._spline_intrados is not None)

    @property
    def spline_extrados(self):
        u"""BezierSpline de l'extrados (BezierSpline ou None)."""
        return self._spline_extrados

    @property
    def spline_intrados(self):
        u"""BezierSpline de l'intrados (BezierSpline ou None)."""
        return self._spline_intrados

    @property
    def extrados(self):
        u"""Points de l'extrados, du BA au BF (x croissant), ndarray(n, 2)."""
        if self._spline_extrados is not None:
            return self._spline_extrados.points
    ...
    def intrados(self):
        ...
        if self._spline_intrados is not None:
            return self._spline_intrados.points
        i_ba = self._leading_edge_index()
    ...
    def from_file(cls, filepath, fmt='auto', unit='mm'):
        ...
        :type filepath: str
        :param fmt: format ('selig', 'lednicer', 'csv', 'bspl',
            'bezier', 'auto')
        :type fmt: str
        :param unit: unite des coordonnees ('mm', 'm', 'normalized')
        :type unit: str
        :returns: instance ProfilSpline
        :rtype: ProfilSpline
        """
        ...
        # Format bspl : traitement separe (cree les splines directement)
        if fmt == 'bspl':
            return cls._read_bspl(filepath)

        # Retrocompatibilite : .bez -> wrap dans BezierSpline mono-segment
        if fmt == 'bezier':
    ...
    def from_naca(cls, designation, n_points=200):
        ...
        :type n_points: int
        :returns: instance ProfilSpline (normalise, corde = 1000 mm)
        :rtype: ProfilSpline
        """
    ...
    def _detect_format(filepath):
        ...
        Heuristiques :
        - .bspl -> format bspl
        - .bez  -> format bezier (retrocompatible)
        ...
        if lower.endswith('.bspl'):
            return 'bspl'
        if lower.endswith('.bez'):
            return 'bezier'
    ...
    def _read_lednicer(filepath):
        ...
        Ligne 3 = vide
        Lignes suivantes = extrados (x y), puis ligne vide,
        puis intrados (x y).
    ...
    def _convert_units(points, unit):
        ...
        if unit == 'mm':
            ...
            raise ValueError(
                u"Unite inconnue '%s'. Attendu : mm, m, normalized"
                % unit)
    ...
    def _write_lednicer(self, filepath):
        ...
        pts = self.points
        i_le = np.argmin(pts[:, 0])
        extrados = pts[:i_le + 1][::-1]  # BA -> BF
        intrados = pts[i_le:]             # BA -> BF
    ...
    def _write_bspl(self, filepath):
        ...
        ou ``continuity`` est la continuite avec le segment suivant
        (C0, C1 ou C2). Le dernier segment de chaque cote n'a pas
        de champ de continuite.
        """
        if not self.has_splines:
            raise ValueError(
                u"Le profil n'est pas en mode spline. "
                u"Utilisez approximate_spline() d'abord.")

        with open(filepath, 'w') as f:
            f.write('%s\n' % self._name)

            for side, spline in [('EXTRADOS', self._spline_extrados),
                                 ('INTRADOS', self._spline_intrados)]:
                segs = spline.segments
                conts = spline.continuities
                f.write('%s %d\n' % (side, len(segs)))
    ...
    @classmethod
    def _read_bspl(cls, filepath):
        u"""Lit un fichier BezierSpline (.bspl).

        :param filepath: chemin du fichier .bspl
        :type filepath: str
        :returns: instance ProfilSpline avec splines
        :rtype: ProfilSpline
        """
        with open(filepath, 'r') as f:
            lines = f.readlines()
        ...
        splines = {}
        for side in ('EXTRADOS', 'INTRADOS'):
            parts = lines[idx].strip().split()
            if parts[0] != side:
                raise ValueError(
                    u"Attendu '%s', lu '%s'" % (side, parts[0]))
            n_segs = int(parts[1])
            idx += 1
            ...
            for k in range(n_segs):
                ...
                if parts[0] != 'SEGMENT':
                    raise ValueError(
                        u"Attendu 'SEGMENT', lu '%s'" % parts[0])
                n_cpts = int(parts[1])
                ...
                for i in range(n_cpts):
                    ...
                    cp.append([float(vals[0]), float(vals[1])])
                idx += n_cpts

                segments.append(Bezier(np.array(cp)))
                if cont is not None:
                    continuities.append(cont)

            splines[side] = BezierSpline(
                segments,
                continuities=continuities if continuities else None)

        # Reconstruire les points discrets (convention Selig)
        ext_pts = splines['EXTRADOS'].points   # BA -> BF
        int_pts = splines['INTRADOS'].points   # BA -> BF
        points = np.vstack([ext_pts[::-1], int_pts[1:]])

        profil = cls(points, name=name)
        profil._spline_extrados = splines['EXTRADOS']
        profil._spline_intrados = splines['INTRADOS']
        profil._output_path = filepath
        profil._output_format = 'bspl'
        return profil

    @classmethod
    def _read_bezier(cls, filepath):
        u"""Lit un fichier Bezier (.bez) en retrocompatibilite.

        Chaque Bezier (mono-segment) est encapsule dans un BezierSpline.

        :param filepath: chemin du fichier .bez
        :type filepath: str
        :returns: instance ProfilSpline avec splines mono-segment
        :rtype: ProfilSpline
        """
        with open(filepath, 'r') as f:
            lines = f.readlines()
        ...
        if parts[0] != 'EXTRADOS':
            raise ValueError(
                u"Attendu 'EXTRADOS', lu '%s'" % parts[0])
        n_ext = int(parts[1])
        idx += 1
        cp_ext = []
        ...
        if parts[0] != 'INTRADOS':
            raise ValueError(
                u"Attendu 'INTRADOS', lu '%s'" % parts[0])
        n_int = int(parts[1])
        idx += 1
        cp_int = []
        ...
        # Creer les Beziers puis les encapsuler dans des BezierSpline
        bez_ext = Bezier(cp_ext)
        bez_int = Bezier(cp_int)

        spl_ext = BezierSpline([bez_ext], name='%s extrados' % name)
        spl_int = BezierSpline([bez_int], name='%s intrados' % name)

        # Reconstruire les points discrets (convention Selig)
        ext_pts = spl_ext.points   # BA -> BF
        int_pts = spl_int.points   # BA -> BF
        points = np.vstack([ext_pts[::-1], int_pts[1:]])

        profil = cls(points, name=name)
        profil._spline_extrados = spl_ext
        profil._spline_intrados = spl_int
        profil._output_path = filepath
        profil._output_format = 'bspl'
        return profil
